...
void QCPDataContainer<DataType>::add(const QCPDataContainer<DataType> &data)
{
    ...
    if (oldSize > 0
            && !qcpLessThanSortKey<DataType>(*constBegin(), *(data.constEnd() - 1))) // prepend if new data keys are all smaller than or equal to existing ones
    {
        if (mPreallocSize < n)
            preallocateGrow(n);
        mPreallocSize -= n;
    ...
    else   // don't need to prepend, so append and merge if necessary
    {
        ...
        if (oldSize > 0
                && !qcpLessThanSortKey<DataType>(*(constEnd() - n - 1), *(constEnd() - n))) // if appended range keys aren't all greater than existing ones, merge the two partitions
...
void QCPDataContainer<DataType>::removeBefore(double sortKey)
{
    ...
    QCPDataContainer<DataType>::iterator itEnd = std::lower_bound(begin(), end(),
...
{
    QCPDataContainer<DataType>::iterator it = std::upper_bound(begin(), end(),
            DataType::fromSortKey(sortKey), qcpLessThanSortKey<DataType>);
...
void QCPDataContainer<DataType>::remove(double sortKeyFrom, double sortKeyTo)
{
    ...
    QCPDataContainer<DataType>::iterator it = std::lower_bound(begin(), end(),
            DataType::fromSortKey(sortKeyFrom), qcpLessThanSortKey<DataType>);
    QCPDataContainer<DataType>::iterator itEnd = std::upper_bound(it, end(),
            DataType::fromSortKey(sortKeyTo), qcpLessThanSortKey<DataType>);
...
void QCPDataContainer<DataType>::limitIteratorsToDataRange(const_iterator &begin,
        const_iterator &end, const QCPDataRange &dataRange) const
{
...
void QCPDataContainer<DataType>::performAutoSqueeze()
{
             ...
             1000)   // below 10 MiB raw data be generous with preallocated memory, below 1k points don't even bother
    {
        ...
        shrinkPreAllocation = mPreallocSize > usedSize *
        ...
        QVariant *details) const
{
    ...
    for (typename QCPDataContainer<DataType>::const_iterator it = begin; it != end; ++it)
    {
        ...
        if (keyRange.contains(mainKey)
                && valueRange.contains(mainValue)) // make sure data point is inside visible range, for speedup in cases where sort key isn't main key and we iterate over all points
        {
...
protected:
    ...
    QVector<QPair<QCPDataRange, QCPDataRange> > getOverlappingSegments(QVector<QCPDataRange>
            thisSegments, const QVector<QPointF> *thisData, QVector<QCPDataRange> otherSegments,
...
protected:
    ...
    virtual void drawStatisticalBox(QCPPainter *painter,
                                    QCPStatisticalBoxDataContainer::const_iterator it, const QCPScatterStyle &outlierStyle) const;
...
public:
    ...
    {
        return QCPRange(low,
                        high);    // open and close must lie between low and high, so we don't need to check them
    }
